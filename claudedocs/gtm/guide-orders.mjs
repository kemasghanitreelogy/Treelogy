/* Order yang pernah klik CTA "Get The Full Size" di /pages/moringa-seed-oil-guide.
 *
 * Membaca atribut order guide_cta_at / guide_cta_page (ditulis buildAttrs di
 * theme.liquid — pola yang sama dengan wa_click_*, lihat TRACKING-MASTER §2d).
 *
 * Pakai:
 *   node claudedocs/gtm/guide-orders.mjs              # 30 hari terakhir
 *   node claudedocs/gtm/guide-orders.mjs 2026-08-20   # sejak tanggal
 *
...
while (hasNext) {
  ...
  for (const o of page.nodes) {
    ...
    if (!attrs.guide_cta_at) continue;
    ...
    matched.push({
      ...
      klik_cta: (attrs.guide_cta_at || '').slice(0, 16).replace('T', ' '),
      halaman: attrs.guide_cta_page || '-',
...
console.log(`Order sejak ${since}: ${scanned} discan, ${matched.length} pernah klik CTA guide\n`);
if (matched.length) console.table(matched);
else console.log('(atribut guide_cta_* baru terisi untuk klik sejak 20 Agu 2026)');
